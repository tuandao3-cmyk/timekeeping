      ...
      <nav className="bg-white border-gray-200 px-4 lg:px-6 py-2.5 border-b-2 shadow-md">
        <div className="flex flex-wrap justify-between items-center mx-auto max-w-screen-xl">
          <Link href="/" className="flex items-center">
            <img src="/img/logohyracap.png" className="mr-3 h-6 sm:h-9" alt="HyraCap Logo" />
          </Link>
            ...
            <ul className="flex flex-col text-sm text-[#000000]/90 leading-6 font-medium lg:flex-row lg:space-x-6 lg:mt-0">
              {menuItems.map((item, index) => (
                <li key={index}>
                  <Link
                    href={item.href}
                    className={`block py-2 pr-4 pl-3 rounded hover:bg-gray-100 lg:hover:bg-transparent lg:p-0 transition duration-300 ${pathname === item.href
