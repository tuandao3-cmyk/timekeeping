...
const ProjectSlider: React.FC = () => {
  ...
  return (
    <div className="w-full relative">
      <h2 className="4xl:text-[72px] 4xl:leading-[80px] 4xl:mb-16 text-3xl font-bold mb-8 text-center">DỰ ÁN ĐANG GỌI VỐN</h2>
      <Swiper
              ...
              <a href='/detail-category' className={`bg-[#07212C] rounded-xl overflow-hidden transition-all duration-300 flex flex-col sm:flex-row ${isActive ? 'scale-100' : 'scale-90'}`}>      
                  ...
                  <div className="w-full h-full rounded-lg overflow-hidden relative group">
                    ...
                    <div className="absolute inset-0 flex items-center justify-center opacity-0 group-hover:opacity-100 transition-opacity duration-300">
                      <button className="4xl:text-[32px] 4xl:leading-[40px] 4xl:py-4 text-white px-4 py-2 rounded-full bg-green-600 transition-colors duration-300">
                        Xem chi tiết
                      </button>
                    </div>
                    </div>
                  ...
                  <div className="md:w-1/2 w-full p-8 flex flex-col justify-between">
                    <div>
                      <span className="4xl:text-[24px] 4xl:leading-[30px] 4xl:left-4 top-4 left-2 bg-[#FFFFFF]/20 text-[#0298F4] px-2 py-1 text-xs rounded flex items-center inline-flex">
                        <FaFlag className="4xl:w-6 4xl:h-6 w-4 h-4 mr-1" />
                        {project.series}
                      </span>
                      <h3 className="4xl:text-[54px] 4xl:leading-[60px] text-2xl font-semibold leading-8 text-white mb-2">{project.name}</h3>
                      <p className="4xl:text-[32px] 4xl:leading-[40px] text-gray-300 leading-6 text-sm mb-4">{project.description}</p>
                    </div>
                    <div>
                      <div className="bg-white rounded-lg p-5 m-3 mb-7">
                        <p className="4xl:text-[24px] 4xl:leading-[30px] text-sm font-inter text-[#000000]/64 mb-1">MỤC TIÊU HUY ĐỘNG</p>
                        <p className="4xl:text-[48px] 4xl:leading-[60px] text-xl leading-6 font-semibold">${project.target.toLocaleString()}</p>
                        <div className="flex justify-between text-xs mt-2">
                          <div>
                            <span className="4xl:text-[36px] 4xl:leading-[40px] font-bold font-inter text-[#31814B] text-sm">${project.raised.toLocaleString()}</span>
                            <span className='4xl:text-[24px] 4xl:leading-[35px] text-[#000000]/64 max-w-[10%] pl-0'> Đã huy động</span>
                          </div>
                          <div>
                            <span className="4xl:text-[36px] 4xl:leading-[40px] font-bold font-inter text-[#31814B] text-sm">{project.percentage}%</span>
                            <span className='4xl:text-[24px] 4xl:leading-[35px] text-[#000000]/64 pl-0'> Hoàn thành</span>
                          </div>
                        </div>
                        <div className="4xl:h-4 4xl:w-full w-full bg-gray-200 rounded-full h-1.5 mt-2">
                          <div 
                            className="4xl:h-4 4xl:w-full w-full bg-[#31814B] h-1.5 rounded-full" 
                      ...
                      <div className="flex gap-2 ml-3">
                        {project.tags.map((tag, index) => (
                          <span key={index} className="4xl:text-[24px] 4xl:leading-[30px] bg-gray-700 text-white text-xs px-2 py-1 rounded">
                            {tag}
                          </span>
                        ))}
                      </div>
      ...
      <div className="4xl:mt-10 4xl:pb-10 flex justify-center mt-6 w-full pb-6">
        <div className="4xl:h-4 bg-gray-200 h-2 rounded-full hidden sm:block w-[85vw] ">
          <div 
            ref={progressBarRef} 
            className="4xl:h-4 bg-[#31814B] h-2 rounded-full transition-all duration-300"
